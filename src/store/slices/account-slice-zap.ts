...
export const loadAccountDetailsZap = createAsyncThunk("account/loadAccountDetails", async ({ networkID, provider, address }: ILoadAccountDetails): Promise<IUserAccountDetails> => {
    let timeBalance = 0;
    let memoBalance = 0;
...
export const calculateUserBondDetailsZap = createAsyncThunk("account/calculateUserBondDetails", async ({ address, bond, networkID, provider }: ICalcUserBondDetails) => {
    if (!address) {
...
export const calculateUserTokenDetailsZap = createAsyncThunk("account/calculateUserTokenDetails", async ({ address, token, networkID, provider }: ICalcUserTokenDetails) => {
    if (!address) {
        return new Promise<any>(resevle => {
            resevle({
                token: "",
...
export interface IAccountSlice {
    zaps: { [key: string]: IUserBondDetails };
    balances: {
        memo: string;
        time: string;
        wmemo: string;
...
const accountSlice = createSlice({
    ...
    extraReducers: builder => {
        builder
            .addCase(loadAccountDetailsZap.pending, state => {
                state.loading = true;
            })
            .addCase(loadAccountDetailsZap.fulfilled, (state, action) => {
                setAll(state, action.payload);
                state.loading = false;
            })
            .addCase(loadAccountDetailsZap.rejected, (state, { error }) => {
                state.loading = false;
                // console.log(error);
            })
            ...
            .addCase(calculateUserBondDetailsZap.pending, (state, action) => {
                state.loading = true;
            })
            .addCase(calculateUserBondDetailsZap.fulfilled, (state, action) => {
                if (!action.payload) return;
                const bond = action.payload.bond;
                state.zaps[bond] = action.payload;
                state.loading = false;
            })
            .addCase(calculateUserBondDetailsZap.rejected, (state, { error }) => {
                state.loading = false;
                // console.log(error);
            })
            .addCase(calculateUserTokenDetailsZap.pending, (state, action) => {
                state.loading = true;
            })
            .addCase(calculateUserTokenDetailsZap.fulfilled, (state, action) => {
                if (!action.payload) return;
                const token = action.payload.token;
                state.tokens[token] = action.payload;
                state.loading = false;
            })
            .addCase(calculateUserTokenDetailsZap.rejected, (state, { error }) => {
                state.loading = false;
                console.log(error);
            });
    },
