...
import { calculateUserBondDetailsZap, getBalances } from "./account-slice-zap";
...
import { exampleBond, wbnb } from "../../helpers/zap";
...
export const calcBondDetailsZap = createAsyncThunk("bonding/calcBondDetails", async ({ bond, value, provider, networkID }: ICalcBondDetails, { dispatch }) => {
    if (!value) {
        value = "0";
    }
...
export const bondAsset = createAsyncThunk("bonding/bondAsset", async ({ value, address, bond, networkID, provider, slippage, useAvax }: IBondAsset, { dispatch }) => {
    ...
    try {
        ...
        await sleep(10);
        await dispatch(calculateUserBondDetailsZap({ address, bond, networkID, provider }));
...
const bondingSlice = createSlice({
    ...
    extraReducers: builder => {
        builder
            .addCase(calcBondDetailsZap.pending, state => {
                state.loading = true;
            })
            .addCase(calcBondDetailsZap.fulfilled, (state, action) => {
                setBondState(state, action.payload);
                state.loading = false;
            })
            .addCase(calcBondDetailsZap.rejected, (state, { error }) => {
                state.loading = false;
                // console.log(error);
            });
    },
